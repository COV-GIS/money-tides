...
return applyOpacity(color);
}`))}const St=me();function ma(a){a.vertex.code.add("#define noPerspectiveWrite(x, w) (x * w)")}function qe(a){a.fragment.code.add("#define noPerspectiveRead(x) (x * gl_FragCoord.w)")}function va(a){return a.pattern.map(e=>Math.round(e*a.pixelRatio))}function ga(a){if(a==null)return 1;const e=va(a);return Math.floor(e.reduce((t,i)=>t+i))}function Sa(a){return a==null?si:a.length===4?a:ge(ba,a[0],a[1],a[2],1)}const ba=Ze();function ya(a,e){if(!e.stippleEnabled)return void a.fragment.code.add(c`float getStippleAlpha(float lineWidth) { return 1.0; }
...
vTransitionType = timeStamps.w;`);const{animation:o}=e;o===3&&n.constants.add("decayRate","float",2.3),n.code.add(c`
    float getTrailOpacity(float x) {
      ${wa(o)}
    }`),n.uniforms.add(new q("timeElapsed",l=>l.timeElapsed),new q("trailLength",l=>l.trailLength),new q("speed",l=>l.animationSpeed),new De("timingOptions",l=>ge(za,l.startTime,l.endTime,l.fadeInTime,l.fadeOutTime))),n.code.add(c`float fadeIn(float x) {
...
return animatedColor;
        ...
        return (exp(-decayRate * x) - cutOff) / (1.0 - cutOff);`;default:return"return 1.0;"}}const za=Ze(),Se=1;function At(a){const e=new sa,{attributes:t,varyings:i,vertex:s,fragment:n}=e,{applyMarkerOffset:o,draped:l,output:r,capType:d,stippleEnabled:p,falloffEnabled:g,roundJoins:m,wireframe:u,innerColorEnabled:f,hasAnimation:y,hasScreenSizePerspective:b}=a;n.include(vi),e.include(fa,a),e.include(ya,a),e.include(gi,a),e.include(Si,a),e.include(Oa,a);const G=o&&!l;G&&(s.uniforms.add(new q("markerScale",v=>v.markerScale)),e.include(Pa,{space:2,hasScreenSizePerspective:b})),bi(s,a),s.uniforms.add(new yi("inverseProjectionMatrix",v=>v.camera.inverseProjectionMatrix),new _i("nearFar",v=>v.camera.nearFar),new q("miterLimit",v=>v.join!=="miter"?0:v.miterLimit),new xi("viewport",v=>v.camera.fullViewport)),s.constants.add("LARGE_HALF_FLOAT","float",65500),t.add("position","vec3"),t.add("previousDelta","vec4"),t.add("nextDelta","vec4"),t.add("lineParameters","vec2"),t.add("u0","float"),i.add("vColor","vec4"),i.add("vpos","vec3",{invariant:!0}),i.add("vLineDistance","float"),i.add("vLineWidth","float");const X=p;X&&i.add("vLineSizeInv","float");const h=d===2,A=p&&h,L=g||A;L&&i.add("vLineDistanceNorm","float"),h&&(i.add("vSegmentSDF","float"),i.add("vReverseSegmentSDF","float")),s.code.add(c`vec2 perpendicular(vec2 v) {
return vec2(v.y, -v.x);
}
